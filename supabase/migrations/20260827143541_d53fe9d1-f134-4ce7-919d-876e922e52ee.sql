SELECT cron.unschedule('ovapass-queue-dispatch-retries');

DROP FUNCTION IF EXISTS public.queue_ovapass_dispatch_retries();
DROP TABLE IF EXISTS public.ovapass_dispatch_queue;
...
SELECT cron.schedule(
  'ovapass-dispatch-searching-trips',
  '* * * * *',
  $$SELECT public.dispatch_searching_ovapass_trips();$$
);